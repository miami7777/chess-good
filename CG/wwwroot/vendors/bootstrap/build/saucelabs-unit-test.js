...
const waitingCallback = (error, body, id) => {
  ...
  if (typeof body !== 'undefined') {
    if (!body.completed) {
      ...
      const platform = CG.platform.join(', ')
      let passed = false
      let errorStr = false

      if (CG.result !== null) {
        if (typeof CG.result === 'string' && errorMessages.includes(CG.result)) {
          errorStr = CG.result
        } else {
          passed = CG.result.total === CG.result.passed
        }
      }
